...
saveRDS(comorbidities, "/rds/general/project/hda_students_data/live/Group7/General/Final/LASSO/comorbidities_selected_inc_comcovbio_test_1.rds")
#Performance evaluation

predict_lasso <- predict(cv_lasso, newx = as(X_test, "dgCMatrix"),y = as.vector(classification) , type = "response", s = cv_lasso$lambda.1se)
print("prediction")
#confusion matrix
confusion_matrix <- table(y_test, predict_lasso > 0.5)
saveRDS(confusion_matrix, "/rds/general/project/hda_students_data/live/Group7/General/Final/LASSO/confusion_matrix_inc_comcovbio_test_1.rds")
pr_lasso <- prediction(predict_lasso, y_test)
prf_lasso <- performance(pr_lasso, measure = "tpr", x.measure = "fpr")
saveRDS(prf_lasso, "/rds/general/project/hda_students_data/live/Group7/General/Final/LASSO/prf_inc_comcovbio_test_1.rds")
#AUC
auc_lasso <- as.numeric(performance(pr_lasso, measure = "auc")@y.values)
saveRDS(auc_lasso, "/rds/general/project/hda_students_data/live/Group7/General/Final/LASSO/auc_inc_comcovbio_test_1.rds")
#ROCR curve
pdf("ROCR_curve_lasso_inc_comcovbio_test_1 .pdf")
